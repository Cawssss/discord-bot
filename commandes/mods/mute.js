...
module.exports.run = async (client, message, args) => { 
    ...
    if(time) {
        ...
        mutetime(user, time, authorcooldown, muterole)
        if(logsmod) logsmod.send({embed:{ description: `**${message.author.username}**#${message.author.discriminator} a retiré la permission de parler dans les salons textuels à **${user.user.username}**#${user.user.discriminator} dans le salon [\`${message.channel.name}\`](https://discord.com/channels/${message.guild.id}/${message.channel.id}) pendant \`${args[1]}\``, color: 3553599, author: { name: "🤫 Mute d'un membre" }, footer: { text: `🕙 ${getNow().time}` } }}) 
        }
