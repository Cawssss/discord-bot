...
module.exports.run = async (client, message, args) => {
    ...
    if (isNaN(time)) return message.channel.send('heure non valide, veuillez réessayer.').then(m => m.delete({ timeout: 5000 }));
    ...
    if (currentCooldown === time) return message.channel.send(`Le slowmode est déjà défini sur ${args[0]}`);
