...
.then(p => message.channel.send(`\`${getNow().time}\` :white_check_mark: ${message.author}, Vous n'avez définis le statut de votre bot en \`${str_content}\`.`))
.catch(e => { message.channel.send(`\`${getNow().time}\` :x: ${message.author}, Une erreur a été rencontré. \n **Plus d'informations:** \`🔻\` \`\`\`${e}\`\`\``); });

} else {
        message.channel.send(`\`${getNow().time}\` :x: ${message.author}, Vous n'avez fournie aucune valeur, veuillez recommencer cette commande en ajoutant ce que vous souhaitez que votre bot ais comme statut`);
    }
};
