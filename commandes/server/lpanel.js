...
message.channel.send(`\`${getNow().time}\` ✨ Création de la catégorie des logs en cours..`).then(msg => {
        m.guild.channels.create('Logs', {
            ...
            c.guild.channels.create('message', {
                ...
                update(message, db)
                m.edit({ embed: { author: { name: `📚 Modification des paramètres à propos des logs de ${message.guild.name}`}, color: db.color, description: "`✨`  Crée une configuration pour moi\n`📥` Définir le salon des logs de l'activité du serveur \n`🔊` Définir le salon des logs des mouvements vocaux\n`👤`  Définir le salon des logs à propos de rôle \n`💭` Définir le salon des logs des messages" , fields: [ {name: "`📥` Logs des entrée", value: db.logs.serveur, inline: true }, { name: "`🔊` Logs vocaux", value: db.logs.vocal, inline: true},{ name: "`👤` Logs des rôles", value: db.logs.role, inline: true}, { name: "`💭` Logs messages", value: db.logs.message, inline: true}   ] } });         
